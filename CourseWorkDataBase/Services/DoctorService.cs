...
public class DoctorService(IDbContextFactory<ApplicationDbContext> dbContextFactory, ILogger<DoctorService> logger)
{
        ...
        try
        {
            ...
            if (medicationId.HasValue)
            {
                medication = await context.Medications
                    .FindAsync(medicationId.Value) ?? throw new InvalidOperationException();
                if (medication == null)
                {
            ...
            else
            {
                ...
                medication = await context.Medications
                    .FirstOrDefaultAsync(s => s.Name == nameMedication) ?? throw new InvalidOperationException();
                if (medication != null)
                {
                ...
                {
                    Name = nameMedication ?? throw new ArgumentNullException(nameof(nameMedication)),
                    Description = descriptionMedication ?? throw new ArgumentNullException(nameof(descriptionMedication))
                };
